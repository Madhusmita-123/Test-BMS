...
{
    public class Fst_Lvl_Rev_Bgt_ApprovalController : Controller
    {
        private readonly IApplicationRepository applicationRepository;
        public Fst_Lvl_Rev_Bgt_ApprovalController(IApplicationRepository appRepository)
        {
            applicationRepository = appRepository;
        }
        // GET: Fst_Lvl_Rev_Bgt_Approval
        ...
        //public ActionResult ViewRequestedBudgetToApprove()
        //{
        //    return View();
        //}
        [HttpPost]
        public ActionResult UpdateRevenueListholevelfirst(List<RevenueSecLevelModel> customers, int id)
        {


            var data = applicationRepository.Updateholevelfrst(customers, id);
        ...
        public ActionResult ViewRequestedBudgetToApprove()
        {
            try
            {
                ...
                {
                    return RedirectToAction("ViewRequestedBudgetToApprove", "Fst_Lvl_Rev_Bgt_Approval");
                }
            }
            catch (Exception ex)
        ...
        public ActionResult GetrevenueDetailsdata(int? Fin_Id, int? Unit_Id, int? Head_Id)
        {
            if (Unit_Id != 0)
            {
                RevenueProposalByUnitModel revenueModel = new RevenueProposalByUnitModel();
                revenueModel.head = applicationRepository.Gethead();
                var data2 = applicationRepository.Getheadfrstlvl(Fin_Id, Unit_Id).FirstOrDefault();
                int Usr_Id = Convert.ToInt32(Session["Usr_Id"].ToString());
                var data = applicationRepository.GetAllRevenueDetailsListfstbudget(Fin_Id, Unit_Id, data2.Head_Id).ToList();
                return Json(data, JsonRequestBehavior.AllowGet);
            }
            else
            {
                int Usr_Id = Convert.ToInt32(Session["Usr_Id"].ToString());
                var data = applicationRepository.GetAllRevenueDetailsListfstbudget(0, 0, 0).ToList();
                return Json(data, JsonRequestBehavior.AllowGet);
            }
        }
    }
